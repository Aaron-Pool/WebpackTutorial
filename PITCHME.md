...
So, first, lets go through the files in the root folder. Starting with the javascript, the component specifically. The order I typically go through is
...
So, first thing we can do is drop the immediately invoked function. That was just a method we were using to avoid accidentally putting variables on the global window object. However, es6 modules are inherently isolated in scope, so we don't have to worry about that. So, let's drop it.
...
And then the parent folder index:
